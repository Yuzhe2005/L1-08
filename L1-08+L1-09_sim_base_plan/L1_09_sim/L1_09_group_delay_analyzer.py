...
def find_latest_h1_csv(data_root: Path = DATA_ROOT) -> Path:
    try:
        return find_latest_stage_file(Path(f"{H1_DATA_DIR_NAME}/together.csv"), data_root=data_root)
    except FileNotFoundError as exc:
        raise FileNotFoundError(f"No together.csv found under {data_root}. Run H1 generation first.") from exc
