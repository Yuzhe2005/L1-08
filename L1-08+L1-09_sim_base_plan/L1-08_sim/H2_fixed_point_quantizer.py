...
from shared_sim.config import get_active_config_value, get_common_config_value
from shared_sim.io_utils import find_latest_stage_file
...
from shared_sim.signal_utils import evaluate_fir_response

from H2_fir_designer import load_h2_target_csv
from l1_08_io import h2_fixed_point_data_dir, h2_target_data_dir, run_dir_from_data_path
...
def find_latest_coefficients_csv() -> Path:
    try:
        return find_latest_stage_file(Path("l1_08_h2_fir_design/h2_fir_coefficients.csv"))
    except FileNotFoundError as exc:
        raise FileNotFoundError(
            f"No h2_fir_coefficients.csv found under {DATA_ROOT}. "
            "Run shared_sim/h1_source.py, H2_target_generator.py, and H2_fir_designer.py first."
        ) from exc
